...
export default function HomePage() {
  ...
  // Sample gig data with IDs
  const excitingGigs = [
    {
      id: 1,
      title: "Dog Walker Needed",
      description:
        "Lorem ipsum dolor sit amet, consectetur adipiscing elit. Aliquam a ultrices mi, a tempor lectus. Quisque...",
      price: 1120,
      createdDays: 2,
    },
    {
  ...
  // Handle view gig navigation
  const handleViewGig = (gigId, gigTitle) => {
    console.log(`Navigating to gig ${gigId}: ${gigTitle}`)
    navigate(`/dashboard/gig-detail/${gigId}`)
  }

  // Handle ongoing gig navigation
  const handleViewOngoingGig = (gigId, gigTitle) => {
    console.log(`Viewing ongoing gig ${gigId}: ${gigTitle}`)
    navigate(`/dashboard/gig/${gigId}`)
  }
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
        <div className="lg:col-span-3">
          ...
          <div className="relative rounded-lg overflow-hidden mb-8">
            <img src="home hero.jpg" alt="Hero background" className="w-full h-48 md:h-64 object-cover" />
          ...
          <div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {excitingGigs.map((gig) => (
                <div key={gig.id} className="bg-white rounded-lg p-4 border border-gray-200">
        ...
        <div className="lg:col-span-1">
          ...
          <div className="bg-white rounded-lg border border-gray-200 p-4 mb-6">
            ...
            <div className="space-y-4">
              {ongoingGigs.map((gig, index) => (
                <div
                  key={gig.id}
                  className={`${index < ongoingGigs.length - 1 ? "border-b border-gray-100 pb-4" : ""}`}
                >
                  <div className="flex items-center gap-2 mb-1">
                    <div className="w-2 h-2 rounded-full bg-red-500"></div>
                    <span className="text-xs text-red-500">{gig.status}</span>
                  </div>
                  <h3 className="font-medium mb-1">{gig.title}</h3>
                  <div className="flex items-center justify-between">
                    <p className="text-gray-600">${formatPrice(gig.price)}</p>
                    <button
                      onClick={() => handleViewOngoingGig(gig.id, gig.title)}
                      className="text-sm px-4 py-1 border border-gray-300 rounded-full hover:bg-gray-50 transition-colors"
                    >
                      View
                    </button>
                  </div>
          ...
          <div className="relative rounded-lg overflow-hidden h-96">
            <img src="/image 8.jpg" alt="Advertisement" className="w-full h-full object-cover" />
